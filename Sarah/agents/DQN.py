...
def target_q(target_network, next_states, rewards, terminals, cumulative_gamma):
  ...
  return jax.lax.stop_gradient(
      rewards + cumulative_gamma * replay_next_qt_max * (1. - terminals))
...
class SarahDQNAgent(object):
  ...
  def unbundle(self, checkpoint_dir, iteration_number, bundle_dictionary):
    ...
    if bundle_dictionary is not None:
      ...
      self._rng = bundle_dictionary['RNG']
...
#     self._rng, rng = jax.random.split(self._rng)
#     self.online_params = self.network_def.init(rng, x=self.state)
#     self.optimizer = create_optimizer(self._optimizer_args)
#     self.optimizer_state = self.optimizer.init(self.online_params)
#     self.target_network_params = self.online_params
