...
class ReplayBuffer(object):
  ...
  def sample_index_batch(self, rng, batch_size):
    ...
    while (len(indices) < batch_size and
      ...
      index = int(jax.random.randint(rng1, [], min_id, max_id) % self._replay_capacity)
  ...
  def sample_transition_batch(self, rng, batch_size=None, indices=None):
    ...
    if indices is None:
      rng, indices = self.sample_index_batch(rng, batch_size)
