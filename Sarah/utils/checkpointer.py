...
def get_latest_checkpoint_number(base_directory,
  ...
  sentinel = os.path.join('checkpoint_*', 'sentinel_{}_complete').format(sentinel_file_identifier)
  glob = os.path.join(base_directory, sentinel)
  regex = re.compile("checkpoint_([-_\d]+)")

  def extract_iteration(x):
    return int(regex.search(x).group(1))
  try:
...
class Checkpointer(object):
  ...
  def _generate_dirname(self, iteration_number):
    dirname = 'checkpoint_{}'.format(iteration_number)
    return os.path.join(self._base_directory, dirname)
  ...
  def _generate_filename(self, file_prefix, iteration_number):
    ...
    dirname = self._generate_dirname(iteration_number)
  ...
  def save_checkpoint(self, iteration_number, data):
    ...
    with tf.io.gfile.GFile(filename, 'wb') as fout:
      fout.write('done')
    # Don't clean up old checkpoints...
